...
export default function ReorderPlanner({ 
  ...
  const getReorderSuggestions = () => {
    ...
    lowStockItems.forEach(stock => {
      ...
      const neededQuantity = (stock.capacity || 0) - (stock.current_stock || 0);
      if (neededQuantity <= 0) return;

      suggestions[key].machines_needing_stock.push({
      ...
      suggestions[key].total_needed += neededQuantity;
      
      if ((stock.current_stock || 0) === 0) {
        suggestions[key].priority = 'critical';
      } else if (suggestions[key].machines_needing_stock.length >= 3 && suggestions[key].priority !== 'critical') {
        suggestions[key].priority = 'high';
      }
    });
      ...
      <Card className="border-0 shadow-md">
        <CardContent className="p-6">
          <div className="flex flex-col md:flex-row justify-between items-start md:items-center gap-4">
            <div>
              <h3 className="text-lg font-semibold text-slate-900">Reorder Planning</h3>
              <p className="text-sm text-slate-600">
                {reorderSuggestions.length} products need restocking across {lowStockItems.length} machine slots
              </p>
            </div>
          ...
          {isLoading ? (
            ...
            <div className="text-center py-12">
              ...
              <p className="text-slate-500 text-sm">
                No products currently need restocking based on par levels.
              </p>
            </div>
          ) : (
