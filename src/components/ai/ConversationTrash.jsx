...
import { 
  ...
  AlertTriangle, 
  Clock,
  RotateCcw // Changed from RestoreIcon
} from 'lucide-react';
...
export default function ConversationTrash() {
      ...
      <Card>
        <CardHeader>
          ...
          <CardDescription>
            ...
            Only admins and support can permanently delete items.
          </CardDescription>
        </CardHeader>
        <CardContent>
          {trashed.length === 0 ? (
                ...
                <Card key={convo.id} className="p-4 flex items-center justify-between">
                  ...
                  <div className="flex gap-2">
                    <Button variant="outline" size="sm" onClick={() => handleRestore(convo.id)}>
                      <RotateCcw className="w-4 h-4 mr-2" /> {/* Changed from RestoreIcon */}
                      Restore
                    </Button>
